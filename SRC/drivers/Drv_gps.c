...
void GPS_data_analysis(void)
{
	...
	Gps_information.last_N_vel = (float)Gps_information.N_vel;							//��¼�ϴ��ϱ����ٶ�
	Gps_information.last_E_vel = (float)Gps_information.E_vel;							//��¼�ϴζ������ٶ�
	
	Gps_information.satellite_num = GPS_data_buff[29];									//��������
	Gps_information.longitude = GPS_data_buff[30] + (GPS_data_buff[31]<<8) + (GPS_data_buff[32]<<16) + (GPS_data_buff[33]<<24);		//����
	Gps_information.latitude  = GPS_data_buff[34] + (GPS_data_buff[35]<<8) + (GPS_data_buff[36]<<16) + (GPS_data_buff[37]<<24);		//γ��
	gps_hmsl                  = GPS_data_buff[42] + (GPS_data_buff[43]<<8) + (GPS_data_buff[44]<<16) + (GPS_data_buff[45]<<24);		//����
	gps_N_vel = Gps_information.N_vel	  = GPS_data_buff[54] + (GPS_data_buff[55]<<8) + (GPS_data_buff[56]<<16) + (GPS_data_buff[57]<<24);		//�����ٶ�
	gps_E_vel = Gps_information.E_vel	  = GPS_data_buff[58] + (GPS_data_buff[59]<<8) + (GPS_data_buff[60]<<16) + (GPS_data_buff[61]<<24);		//�����ٶ�
	gps_D_vel                             = GPS_data_buff[62] + (GPS_data_buff[63]<<8) + (GPS_data_buff[64]<<16) + (GPS_data_buff[65]<<24);		//�����ٶ�
	
	Gps_information.N_vel /=10;								//��λ���� cm/s
	Gps_information.E_vel /=10;								//��λ���� cm/s	

	if (Gps_information.satellite_num >= 6 && Gps_information.new_pos_get == 0)			//������������6��,�ҵ�һ�λ�ȡ��γ�ȵ�
	{
...
void GPS_Data_Processing_Task(u8 dT_ms)
{
					...
					&& flag.flight_mode >= LOC_HOLD)//CH_N[AUX1] > -200 )
	{
		...
		{
			switchs.gps_on = 1;							//����GPSģʽ
		}
	}
	else
	{
		switchs.gps_on = 0;							//�ر�GPSģʽ
	}
	//û����ɵ�ʱ����ܼ�¼������
	if (!flag.taking_off)
	{
		if (switchs.gps_on)
		{
			Gps_information.hope_latitude = Gps_information.latitude_offset;		//��¼����λ��Ϊ��ǰλ��
			Gps_information.hope_longitude = Gps_information.longitude_offset;
			Gps_information.home_latitude = Gps_information.latitude_offset;		//��¼������λ��
	...
	else
	{
		...
		{	
			if (flag.flight_mode == RETURN_HOME || flag.rc_loss_back_home )//(CH_N[AUX1] > 200)
			{
				...
				{
					Gps_information.Back_home_f = TRUE;		//�򿪷���
					Gps_information.hope_latitude = Gps_information.home_latitude;			//�ı�����λ��Ϊ��ɵ�λ��
			...
			else
			{
				if (Gps_information.Back_home_f)			
				{
					Gps_information.Back_home_f = FALSE;	//�رշ���
					Gps_information.hope_latitude = Gps_information.latitude_offset;		//�ı�����λ��Ϊ��ǰλ��
					Gps_information.hope_longitude = Gps_information.longitude_offset;
				}
			}
		}			
